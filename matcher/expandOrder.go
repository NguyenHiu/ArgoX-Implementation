...
func (e *ExpandOrder) Encode_TransferBatching() ([]byte, error) {
	...
	if err := binary.Write(data, binary.BigEndian, s); err != nil {
		return nil, err
	}
	...
	if err := binary.Write(data, binary.BigEndian, tData.Bytes()); err != nil {
		return nil, err
	}
	...
	if err := binary.Write(data, binary.BigEndian, o); err != nil {
		return nil, err
	}
...
func (e *ExpandOrder) IsValidOrder(owner common.Address) bool {
	...
	return new(big.Int).Sub(e.OriginalOrder.Amount, amount).Cmp(e.ShadowOrder.Amount) != -1
}
