...
do
    ...
    do
        ...
        ganache_pid=$!
        sleep 2
        go run . 8545 1337 "${no_matcher[i]}" "${no_send_to[i]}" run $data_file ./data/result/_fix_1/price_curve_"${no_send_to[i]}"_"${no_matcher[i]}"_$n > ./data/test_2/price_curve_"${no_send_to[i]}"_"${no_matcher[i]}"_$n/log
        kill $ganache_pid 2>/dev/null
